...
export function Dossier({
  ...
  const far = selection.type === "li" ? liTown(selection.id) : undefined;
  const county =
  ...
  const title = far?.name ?? place?.name ?? county?.name ?? OVERVIEW.name;
  const summary = far?.summary ?? place?.summary ?? county?.summary ?? OVERVIEW.summary;
  const body = far?.body ?? place?.body ?? county?.body ?? OVERVIEW.body;
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col bg-elevated text-paper shadow-border">
      ...
      <div className="flex flex-wrap items-center gap-2 px-5 pb-4">
        ...
        {far ? <Badge>{far.inland ? "Inland" : "Sound"}</Badge> : null}
        {far ? <Badge variant="paper">off chronicle</Badge> : null}
        {place?.families ? (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {place?.id === "bwc-da" || place?.id === "silent-city" ? (
          <img
              ...
              {liHits.map((t) => (
                <li key={t.id}>
                  <button
                    ...
                    className={cn(
                      "flex w-full items-start justify-between gap-2 rounded-lg px-2 py-2 text-left hover:bg-subtle",
                      far?.id === t.id && "bg-subtle",
                    )}
                  >
                    <span>
                      <span className="block text-sm text-paper">{t.name}</span>
                      <span className="block text-xs text-muted">Long Island</span>
                    </span>
                  </button>
                </li>
              ))}
